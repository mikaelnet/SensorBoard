...
float BMP085_readAltitude(float atmospheric, float sealevelPressure)
{
    return 44330 * (1.0 - pow(atmospheric /sealevelPressure,0.1903));
}
...
float BMP085_seaLevelForAltitude(float altitude, float atmospheric, float temperature)
{
    ...
    /*        T   = temperature (in ?C)           */
    return (float)pow((((altitude*0.0065)/(temperature + 273.15F))+1), (1.0/0.190223F))*atmospheric;
}
