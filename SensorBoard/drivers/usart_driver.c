...
/*! \brief Thest if the send buffer is empty
 *
 *  \retval true	The buffer is empty
 *  \retval false	The buffer contains data
 */ 
bool USART_TXBuffer_IsEmpty(USART_data_t *usart_data)
{
	return usart_data->buffer.TX_Head == usart_data->buffer.TX_Tail;
}
